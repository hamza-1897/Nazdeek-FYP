...
const PaymentUploadScreen = ({ route, navigation }) => {
  ...
  const { 
    type = 'registration', 
    selectedPlan = null, 
    bankAccounts: passedBankAccounts = null 
  } = route.params || {};

  const [image, setImage] = useState(null);
  const [loading, setLoading] = useState(false);
  const [fetching, setFetching] = useState(type === 'registration'); 
  const [amount, setAmount] = useState('');
  const [bankAccounts, setBankAccounts] = useState(passedBankAccounts || []);
  ...
  useEffect(() => {
    if (isRegistration) {
      const fetchDetails = async () => {
        try {
          ...
          if (response && response.success) {
            setAmount(response.registrationFeeAmount);
      ...
      if (selectedPlan) {
        setAmount(`${selectedPlan.price} (${selectedPlan.title})`);
      }
    }
  }, [type]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!isRegistration && selectedPlan) {
        formData.append('planId', selectedPlan.id || selectedPlan._id);
    ...
    <SafeAreaView className="flex-1 bg-white">
      <View className="px-6 py-4 flex-row items-center justify-between border-b border-gray-100 bg-white">
        ...
        <Text className="text-lg font-bold text-gray-900">
          {isRegistration ? 'Registration Fee' : 'Subscription Payment'}
        </Text>
        <View className="w-6" />
      </View>
      ...
      <ScrollView contentContainerStyle={{ padding: 24 }}>
        <Text className="text-3xl font-bold text-[#1a5ea1] text-center mb-1">
          {isRegistration ? 'Registration Fee' : 'Premium Upgrade'}
        </Text>
        ...
        <Text className="text-gray-500 text-center mb-8 text-base">
          {isRegistration 
            ...
            : 'Deposit subscription fee to unlock premium features'}
        </Text>
