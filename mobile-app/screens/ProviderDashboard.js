    ...
    <View className="flex-1 bg-white">
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1 px-6">
        ...
        <View className="flex-row justify-between">
          <ActionItem icon="diff-added" label="Add service" color="#1a5ea1" />
          <ActionItem icon="calendar" label="Bookings" color="#b45309" />
        </View>

        <Text className="text-gray-800 font-bold text-lg mt-8 mb-4 uppercase tracking-wider text-[10px]">Today's Bookings</Text>
        
        <View className="bg-white border border-gray-100 rounded-2xl p-4 flex-row items-center shadow-sm mb-10">
      ...
      <View className="flex-row justify-around py-4 border-t border-gray-100 bg-white">
        <TabItem icon="home-outline" label="Home" active={true} />
        <TabItem icon="add-circle-outline" label="Services" />
        <TabItem icon="briefcase-outline" label="Jobs" />
...
const ActionItem = ({ icon, label, color }) => (
  <TouchableOpacity className="w-[48%] bg-white border border-gray-100 p-4 rounded-2xl mb-4 flex-row items-center shadow-sm">
    <View className="mr-3">
