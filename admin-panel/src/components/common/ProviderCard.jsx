...
export default function ProviderCard({ provider, onViewDetails }) {
  ...
  const image = provider?.providerImage || '';
        ...
        <div className="flex items-center justify-between gap-4 mb-4">
          <div className="flex items-center gap-4 flex-1 min-w-0">
            ...
            <div className="flex-1">
              <h3 className="font-bold text-slate-800 text-base lg:text-lg leading-snug">
                {name}
              </h3>
          ...
          <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold capitalize flex-shrink-0 ${currentStatus.classes}`}>
            {currentStatus.icon}
            {currentStatus.text}
          </span>
        </div>
